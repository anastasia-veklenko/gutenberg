...
/**
 * Various functions required for test-foundation to work properly
 *
 *
 */
/*
 ...
 */
if ( ! class_exists( 'test_navwalker' ) ) :
    class test_navwalker extends Walker_Nav_Menu {
        function start_lvl( &$output, $depth = 0, $args = array() ) {
            $indent = str_repeat("\t", $depth);
            $output .= "\n$indent<ul class=\"dropdown menu vertical\" data-toggle>\n";
        }
    }
    if ( ! class_exists( 'test_mobile_navwalker' ) ) :
        class test_mobile_navwalker extends Walker_Nav_Menu {
            function start_lvl( &$output, $depth = 0, $args = array() ) {
                $indent = str_repeat("\t", $depth);
 ...
 */
if ( ! function_exists( 'test_menu_fallback' ) ) :
    function test_menu_fallback() {
...
if ( ! function_exists( 'test_active_nav_class' ) ) :
    function test_active_nav_class( $classes, $item ) {
        if ( 1 == $item->current || true == $item->current_item_ancestor ) {
            $classes[] = 'active';
        }
        return $classes;
    }
    add_filter( 'nav_menu_css_class', 'test_active_nav_class', 10, 2 );
endif;
 ...
 */
if ( ! function_exists( 'test_active_list_pages_class' ) ) :
    function test_active_list_pages_class( $input ) {
        $pattern = '/current_page_item/';
        $replace = 'current_page_item active';
        $output = preg_replace( $pattern, $replace, $input );
        return $output;
    }
    add_filter( 'wp_list_pages', 'test_active_list_pages_class', 10, 2 );
endif;

// 4 - Comments tree: @link {https://codex.wordpress.org/Function_Reference/Walker_Comment}

if ( ! class_exists( 'test_Comments' ) ) :
    class test_Comments extends Walker_Comment{
